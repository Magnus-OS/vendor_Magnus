...
- githubremote:    Add git remote for MagnusOS Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/MagnusOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.magnus.device | grep -q "$MAGNUS_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/Magnus-OS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.magnus.device | grep -q "$MAGNUS_BUILD");
    then
        ...
        echo "The connected device does not appear to be $MAGNUS_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.magnus.device | grep -q "$MAGNUS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
